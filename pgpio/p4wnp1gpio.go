...
func (p *P4wnp1PinIO) startEdgeDetection(pull gpio.Pull, edge gpio.Edge, preserveUnconsumed bool) (err error) {
	fmt.Println("starting edge detection ...")
	...
	Loop:
		...
		fmt.Println("... edge detection stopped")
	}()


	fmt.Println("... edge detection started")
	return
}
...
func (p *P4wnp1PinIO) stopEdgeDetection() error {
	fmt.Println("stopping edge detection ...")
...
func (p *P4wnp1PinIO) In(pull gpio.Pull, edge gpio.Edge) (err error) {
	...
	if err != nil { return }
	// bring up edgeDetection go routine if needed
	p.stopEdgeDetection()
	if edge != gpio.NoEdge {
